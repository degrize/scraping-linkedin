...
class LinkedCompanySpider(scrapy.Spider):
    ...
    def parse_response(self, response):
        ...
        company_item['name'] = response.css('.ember-view.t-24.t-black.t-bold.full-width span::text').get(default='not-found').strip()
        company_item['summary'] = response.css('.org-top-card-summary-info-list__info-item::text').get(default='not-found').strip()

        try:
            ## all company details 
            company_details = response.css('.overflow-hidden .mb1.text-heading-small')
